...
var INTERVAL   = settings.lfg.update_interval;
...
module.exports = {
    ...
    init: (conf) => {
        settings = conf;
        INTERVAL = settings.lfg.update_interval;
    },
     ...
     *  @param bot
     *  @param id - LFG id
     *  @param userid - User id to add into LFG party
     */
     ...
     *  @param bot
     *  @param id - LFG id
     *  @param userid - User id to remove from LFG party 
     */
    ...
    timeout: function(bot, id) {
        try {
            ...
            ch.fetchMessage(e.id).then(message => {
                ...
                message.edit({ embed }).then(() => {
                    console.log(chalk.bgYellow.black(`[${moment().format(settings.timeFormat)}] ${e.party_leader_name}'s LFG for ${e.game} has timed out.`));
                }).catch(err => console.log(err));
            }).catch(err => console.log(err));
        } catch (err) {
            console.log(err);
     ...
     *  @param bot
     *  @param id - LFG id
     *  @param timeLeft - Minutes remaining on LFG party
     */
    warning: function(bot, id, timeLeft) {
        try {
            ...
            let ch = bot.channels.get(e.channel);
            ch.send({ embed });
            console.log(chalk.bgYellow.black(`[${moment().format(settings.timeFormat)}] ${e.party_leader_name}'s LFG is running out of time.`));
        } catch (err) {
            console.log(err);
        }
    },
    ...
    complete: function(bot, id) {
        try {
            ...
            bot.lfgStack.delete(id);
            if (bot.lfgStack.size === 0) bot.lfgUpdate(false);
            console.log(chalk.bgYellow.black(`[${moment().format(settings.timeFormat)}] ${e.party_leader_name}'s LFG party for ${e.game} is full.`));
        } catch (err) {
            console.log(err);
        }
    },
     ...
     *  @param bot
     *  @param id - LFG id
     *  @param removed - Whether the LFG party message was removed or not, which could have triggered this func
     */
    cancel: function(bot, id, removed) {
        try {
            ...
            if (removed) {
                bot.lfgStack.delete(id);
                if (bot.lfgStack.size === 0) bot.lfgUpdate(false);
                return console.log(chalk.bgYellow.black(`[${moment().format(settings.timeFormat)}] ${e.party_leader_name}'s LFG party for ${e.game} has been deleted.`));
            }
            let ch = bot.channels.get(e.channel);
            ch.fetchMessage(e.id).then(message => {
                message.edit({ embed }).then(() => {
                    bot.lfgStack.delete(id);
                    if (bot.lfgStack.size === 0) bot.lfgUpdate(false);
                    console.log(chalk.bgYellow.black(`[${moment().format(settings.timeFormat)}] ${e.party_leader_name}'s LFG party for ${e.game} has been cancelled.`));
                }).catch(err => console.log(err));
            }).catch(err => {
                console.log(err)
            });
